      ...
      body: BlocConsumer<FavDbCubit, FavDbState>(
          builder: (context, state) {
            switch (state.detailStatus) {
              case FavDetailStatus.initial:
                return const WeatherInitialWidget();
              case FavDetailStatus.loading:
                return const WeatherLoading();
              case FavDetailStatus.success:
                int? isDay = state.weather!.current!.isDay;
                return Stack(
                  children: [
                    if (isDay == 0)
                      Image.asset(nightBgAssets(state.weather!.current!.conditionText ?? ""),
                          fit: BoxFit.fill,
                          width: size.width,
                          height: size.height,
                          errorBuilder: (_, __, ___) => Container()),
                    if (isDay == 1)
                      Image.asset(dayBgAssets(state.weather!.current!.conditionText ?? ""),
                          fit: BoxFit.fill,
                          width: size.width,
                          height: size.height,
                          errorBuilder: (_, __, ___) => Container()),
          ...
          listener: (context, state) {
            print("Status:: ${state.detailStatus}");
